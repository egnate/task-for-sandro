        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    ...
                    <p class="text-gray-500 text-sm">Make changes to your note</p>
                </div>
            </div>
            <div class="flex items-center space-x-3">
                @if($note->is_published && $note->slug)
                    <a href="{{ route('public.note', $note->slug) }}" target="_blank"
                       class="inline-flex items-center px-4 py-2.5 text-blue-600 hover:text-blue-700 bg-blue-50 hover:bg-blue-100 rounded-xl transition-all duration-200">
        ...
        <div class="flex items-center justify-between bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
            <div class="flex items-center space-x-6">
                ...
                @if($currentImage && !$image)
                    <div class="flex items-center space-x-3">
                        <div class="relative">
                            <img src="{{ Storage::url($currentImage) }}" alt="Current" class="w-16 h-16 object-cover rounded-xl">
                            <button type="button" wire:click="removeCurrentImage" wire:confirm="Remove image?"
                ...
                @if($image)
                    <div class="flex items-center space-x-3">
                        ...
                        <span class="text-sm text-emerald-600 font-medium">New image</span>
                    </div>
                @endif
                ...
                @if(($currentImage && !$image) || $image)
                    <label for="image-upload" class="cursor-pointer">
                        <div class="flex items-center space-x-2 px-3 py-2 text-gray-500 hover:text-gray-700 hover:bg-gray-50 rounded-xl transition-all">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                            </svg>
